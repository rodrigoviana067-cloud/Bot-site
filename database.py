...
def init_database():
    with get_db() as db:
        ...
        cur.execute("CREATE TABLE IF NOT EXISTS autopost_control (user_id INTEGER PRIMARY KEY, last_post_at TIMESTAMP, paused_until TIMESTAMP, posts_today INTEGER DEFAULT 0, error_count INTEGER DEFAULT 0, total_posts INTEGER DEFAULT 0)")
        ...
        cur.execute("CREATE TABLE IF NOT EXISTS remarketing (id SERIAL PRIMARY KEY, user_id INTEGER, click_id INTEGER, etapa INTEGER DEFAULT 1, status TEXT DEFAULT 'pendente', scheduled_at TIMESTAMP, sent_at TIMESTAMP, message TEXT)")
